...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("gaPropertyId").value = localStorage.getItem("markGaPropertyId") || PROPERTY_DEFAULT;
  $("gaClientId").value = localStorage.getItem("markGaClientId") || "";
...
function connectGoogle() {
  ...
  if (!window.google?.accounts?.oauth2) {
    showError("Google Identity Services has not loaded yet. Wait a few seconds and try again.");
    return;
  }
  ...
  tokenClient = google.accounts.oauth2.initTokenClient({
    ...
    error_callback: err => {
      showError(`Google sign-in could not complete: ${err.type || "unknown error"}`);
    }
  });
...
async function loadDashboard() {
  ...
  clearError();
  $("analyticsDashboard").setAttribute("aria-busy", "true");
  $("analyticsDashboard").classList.add("loading");

  try {
    const startDate = selectedDays === 1 ? "today" : `${selectedDays - 1}daysAgo`;
    ...
    const [
      ...
      runReport({
        ...
        metrics: [{ name: "gaActiveUsers" }],
        orderBys: [{ dimension: { dimensionName: "date" } }]
      }),
      rankedReport(startDate, "country", "gaActiveUsers", 8),
      rankedReport(startDate, "deviceCategory", "gaActiveUsers", 8),
      rankedReport(startDate, "browser", "gaActiveUsers", 8),
      runReport({
        dateRanges: [{ startDate, endDate: "today" }],
      ...
      runRealtime({ metrics: [{ name: "gaActiveUsers" }] }),
      runRealtime({
        dimensions: [{ name: "country" }],
        metrics: [{ name: "gaActiveUsers" }],
        orderBys: [{ metric: { metricName: "gaActiveUsers" }, desc: true }],
        limit: "8"
      })
    ]);
    ...
    $("gaLastUpdated").textContent = `Updated ${new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}`;
  } catch (error) {
    if (error.status === 401) {
      accessToken = "";
      $("gaConnectionStatus").textContent = "Session expired";
      $("gaConnectionStatus").classList.remove("connected");
      showError("Your Google access session expired. Click Connect Google Analytics again.");
    } else {
      showError(error.message || String(error));
    }
  } finally {
    $("analyticsDashboard").setAttribute("aria-busy", "false");
    $("analyticsDashboard").classList.remove("loading");
  }
}
...
async function gaFetch(method, body) {
  const propertyId = $("gaPropertyId").value.trim();
  const response = await fetch(`${API_BASE}${encodeURIComponent(propertyId)}:${method}`, {
    method: "POST",
    headers: {
...
function paintTrend(report) {
  ...
  if (!rows.length) {
    container.className = "analytics-bar-chart analytics-empty";
    container.textContent = "No data for this period yet.";
    return;
  }
  ...
  container.innerHTML = points.map(point => {
    const h = Math.max(2, Math.round((point.value / max) * 180));
    const label = formatDate(point.date);
    return `<div class="analytics-bar-column">
      <div class="analytics-bar" style="height:${h}px" data-tip="${escapeHtml(label)} · ${point.value} users"></div>
...
function paintRanking(id, report) {
  ...
  const rows = report.rows || [];
  if (!rows.length) {
    container.className = "analytics-ranking analytics-empty";
  ...
  container.innerHTML = rows.map((row, i) => {
    ...
    return `<div class="analytics-rank-row">
      <span class="analytics-rank-label">${escapeHtml(label)}</span>
      <strong class="analytics-rank-value">${number(value)}</strong>
      <div class="analytics-meter"><i style="width:${Math.max(3,(value/max)*100)}%"></i></div>
    </div>`;
  }).join("");
}
...
function paintEvents(report) {
  ...
  const actions = [];
  const interests = { low: 0, medium: 0, high: 0 };

  (report.rows || []).forEach(row => {
    const name = row.dimensionValues?.[0]?.value || "";
    const count = Number(row.metricValues?.[0]?.value || 0);

    if (name.startsWith("contact_") && actionNames[name]) {
      actions.push({ name, label: actionNames[name], count });
    }
    if (name === "interest_low") interests.low += count;
    if (name === "interest_medium") interests.medium += count;
    if (name === "interest_high") interests.high += count;
  });

  const totalActions = actions.reduce((sum, item) => sum + item.count, 0);
  $("gaContactActions").textContent = number(totalActions);

  const actionsContainer = $("gaActions");
  if (!actions.length) {
    actionsContainer.className = "analytics-action-list analytics-empty";
    actionsContainer.textContent = "Action-specific tracking starts with this version. Perform a few actions on the public card, then refresh.";
  } else {
    actions.sort((a,b) => b.count - a.count);
    actionsContainer.className = "analytics-action-list";
    actionsContainer.innerHTML = actions.map(item => `
      <div class="analytics-action-row">
        <span class="analytics-action-label">${escapeHtml(item.label)}</span>
        <strong class="analytics-action-value">${number(item.count)}</strong>
      </div>`).join("");
  }

  const interestTotal = interests.low + interests.medium + interests.high;
  let level = "No signal yet";
  if (interestTotal) {
    const weighted = (interests.low + interests.medium*2 + interests.high*3) / interestTotal;
    level = weighted >= 2.35 ? "High" : weighted >= 1.55 ? "Medium" : "Low";
  }
  ...
  if (!interestRows.length) {
    container.className = "ranking compact empty";
    container.textContent = "No interest events collected yet.";
  } else {
    const max = Math.max(...interestRows.map(([,count]) => count), 1);
    container.className = "ranking compact";
    container.innerHTML = interestRows.map(([label,count]) => `
      <div class="analytics-rank-row">
        <span class="analytics-rank-label">${label}</span>
        <strong class="analytics-rank-value">${number(count)}</strong>
        <div class="analytics-meter"><i style="width:${Math.max(3,(count/max)*100)}%"></i></div>
      </div>`).join("");
  }
}

function paintLive(live, countries) {
  const liveCount = Number(live.rows?.[0]?.metricValues?.[0]?.value || 0);
  $("gaLiveUsers").textContent = number(liveCount);
  paintRanking("gaLiveCountries", countries);
}
...
function formatDate(raw) {
  ...
  return new Date(`${y}-${m}-${d}T00:00:00`).toLocaleDateString([], {month:"short",day:"numeric"});
}
...
function clearError() {
  $("gaErrorPanel").classList.add("is-hidden");
  $("gaErrorPanel").textContent = "";
}
