...
async function loadDashboard() {
  ...
  try {
    ...
    if (error.status === 401) {
      ...
      $("gaConnectionStatus").textContent =
        "Session expired";
...
function paintSummary(report) {
  ...
  $("gaActiveUsers").textContent =
    number(
      row[0]?.value
    );

  $("gaSessions").textContent =
    number(
      row[1]?.value
    );
